...
#staff = conf_staff.merge(otrs_staff, on=['Фамилия', 'Имя', 'E-mail'], how = 'outer')
...
##staff['E-mail_x'] = staff[['E-mail_x','E-mail_y']].fillna('').sum(axis=1)
#staff['Моб. номер_x'] = staff[['Моб. номер_x','Моб. номер_y']].fillna('').sum(axis=1)
#staff = staff.drop(staff.columns[[6, 7, 8]], axis='columns')
#staff = pd.read_excel(temppath + '\FillPhones\ADT_staff.xlsx', encoding = 'utf-8')
#staff.to_excel(temppath + '\FillPhones\ADT_staff_test.xlsx', index=False, header=["Логин", "Фамилия", "Имя", "Внутренний номер", "E-mail", "Моб. номер"])
staff = pd.read_excel(temppath + '\FillPhones\ADT_staff_test.xlsx', encoding = 'utf-8')
print(staff.groupby(['Фамилия', 'Имя'], group_keys=False).agg('sum'))
staff.to_excel(temppath + '\FillPhones\ADT_staff_test.xlsx', index=False, header=["Логин", "Фамилия", "Имя", "Внутренний номер", "E-mail", "Моб. номер"])
